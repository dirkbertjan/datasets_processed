...
class MFTC(AbstractTask):
    task = "mftc"
    num_labels =11
    labels_list = ['fairness', 'non-moral','purity', 'degradation','loyalty', 'care','cheating','betrayal','subversion','authority',' harm']
    metric = [metrics.accuracy, metrics.f1_macro]

    def load_datasets(self):
        dataset_args = {"cache_mode": "offline"}
        print("task ", self.task)
        data_dir = join(self.data_dir, self.task) 
        data_files = {
            "train": join(data_dir, "train.json"),
        ...
        return load_dataset("json", data_files=data_files, cache_dir=self.cache_dir, clear_cache=True, **dataset_args)
